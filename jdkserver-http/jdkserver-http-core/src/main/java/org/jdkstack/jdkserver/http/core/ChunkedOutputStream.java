...
/**
 * a class which allows the caller to write an arbitrary number of bytes to an underlying stream.
 * normal close() does not close the underlying stream
 *
 * <p>This class is buffered.
 *
 * <p>Each chunk is written in one go as :- abcd\r\nxxxxxxxxxxxxxx\r\n
 *
 * <p>abcd is the chunk-size, and xxx is the chunk data If the length is less than 4 chars (in size)
 * then the buffer is written with an offset. Final chunk is: 0\r\n\r\n
 */
class ChunkedOutputStream extends FilterOutputStream {
  ...
  /**
   * write out a chunk , and reset the pointers chunk does not have to be CHUNK_SIZE bytes count
   * must == number of user bytes (<= CHUNK_SIZE)
   */
