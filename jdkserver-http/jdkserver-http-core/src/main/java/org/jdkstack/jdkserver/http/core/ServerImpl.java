...
import org.jdkstack.jdkserver.http.core.HttpConnection.State;
import org.jdkstack.jdkserver.http.core.exception.HttpError;
...
public class ServerImpl implements TimeSource {
  ...
  ServerImpl(HttpServer wrapper, String protocol, InetSocketAddress addr, int backlog)
    ...
    this.logger = System.getLogger("org.jdkstack.jdkhttpserver.core.httpserver");
  ...
  class Dispatcher implements Runnable {
    ...
    private void handleEvent(Event r) {
      ...
      try {
        ...
        logger.log(Level.TRACE, "Dispatcher (1)", e);
        c.close();
      }
    }
    ...
    public void handle(SocketChannel chan, HttpConnection conn) {
      try {
        ExchangeTask t = new ExchangeTask(chan, protocol, conn);
        executor.execute(t);
      } catch (HttpError e1) {
  ...
  class ExchangeTask implements Runnable {
    ...
    ExchangeTask(SocketChannel chan, String protocol, HttpConnection conn) throws IOException {
      this.chan = chan;
    ...
    public void run() {
      ...
      try {
        if (context != null) {
          ...
          if (https) {
            if (sslContext == null) {
              logger.log(Level.WARNING, "SSL connection received. No https context created");
        ...
        if (newconnection) {
          connection.setParameters(
              rawin, rawout, chan, engine, sslStreams, sslContext, protocol, ctx, rawin);
        }
