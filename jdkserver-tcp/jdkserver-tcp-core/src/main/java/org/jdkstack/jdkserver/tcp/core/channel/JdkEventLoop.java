...
public class JdkEventLoop extends SingleThreadEventLoop {
  ...
  private SelectorTuple openSelector() {
              ...
              public Object run() {
                try {
                  return Class.forName(
                      "sun.nio.ch.SelectorImpl", false, PlatformDependent.getSystemClassLoader());
    ...
    if (!(maybeSelectorImplClass instanceof Class)
      ...
      if (maybeSelectorImplClass instanceof Throwable) {
        ...
        // logger.trace("failed to instrument a special java.util.Set into: {}", unwrappedSelector,
        // t);
      }
      return new SelectorTuple(unwrappedSelector);
    }
    ...
    return new SelectorTuple(
        unwrappedSelector, new SelectedSelectionKeySetSelector(unwrappedSelector, selectedKeySet));
  }

  /**
   * Returns the {@link SelectorProvider} used by this {@link NioEventLoop} to obtain the {@link
   * Selector}.
   */
  ...
  public void register(final SelectableChannel ch, final int interestOps, final NioTask<?> task) {
    ...
    if (inEventLoop()) {
      ...
      try {
        ...
        // may block for a long time while trying to obtain an internal lock that may be hold while
        // selecting.
        submit(
                new Runnable() {
                  @Override
  ...
  private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
    ...
    if (!k.isValid()) {
      ...
      try {
        ...
        // If the channel implementation throws an exception because there is no event loop, we
        // ignore this
        // because we are only trying to determine if ch is registered to this event loop and thus
        // has authority
        // to close ch.
        return;
      }
      // Only close ch if ch is still registered to this EventLoop. ch could have deregistered from
      // the event loop
      // and thus the SelectionKey could be cancelled as part of the deregistration process, but the
      // channel is
    ...
    try {
      ...
      // We first need to call finishConnect() before try to trigger a read(...) or write(...) as
      // otherwise
      ...
      if ((readyOps & SelectionKey.OP_WRITE) != 0) {
        // Call forceFlush which will also take care of clear the OP_WRITE once there is nothing
        // left to write
