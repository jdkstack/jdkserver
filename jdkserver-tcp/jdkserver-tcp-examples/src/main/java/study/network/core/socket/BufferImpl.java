...
public class BufferImpl implements Buffer {
  ...
  BufferImpl(int initialSizeHint) {
    buffer = TcpByteBufAllocator.DEFAULT.heapBuffer(initialSizeHint, Integer.MAX_VALUE);
  }

  BufferImpl(byte[] bytes) {
    buffer =
        TcpByteBufAllocator.DEFAULT.heapBuffer(bytes.length, Integer.MAX_VALUE).writeBytes(bytes);
  }
  ...
  public ByteBuf getByteBuf() {
    ...
    if (buffer.getClass() != TcpHeapByteBuf.class
        && buffer.getClass() != TcpUnsafeHeapByteBuf.class) {
