 ...
 *
 * <p>It makes use of the JSSE framework, and specifically the {@link SSLEngine} logic, which is
 ...
 *
 * <p>{@link NioSslPeer} implements the handshake protocol, required to establish a connection
 * between two peers, which is common for both client and server and provides the abstract {@link
 * NioSslPeer#read(SocketChannel, SSLEngine)} and {@link NioSslPeer#write(SocketChannel, SSLEngine,
 * String)} methods, that need to be implemented by the specific SSL/TLS peer that is going to
 * extend this class.
 *
...
public abstract class NioSslPeer {
   ...
   * SSLEngine#wrap(ByteBuffer, ByteBuffer)} is applied on {@link NioSslPeer#myAppData}. It should
   * be initialized using {@link SSLSession#getPacketBufferSize()}, which returns the size up to
   * which, SSL/TLS packets will be generated from the engine under a session. All SSLEngine network
   * buffers should be sized at least this large to avoid insufficient space problems when
   ...
   * application data from any peer. Can be initialized with {@link
   * SSLSession#getApplicationBufferSize()} for an estimation of the other peer's application data
   * and should be enlarged if this size is not enough.
   */
  ...
  protected abstract void write(SocketChannel socketChannel, SSLEngine engine, String message)
      throws Exception;
   ...
   *
   * <p>A typical handshake will usually contain the following steps:
   *
   * <ul>
   *   <li>1. wrap: ClientHello
   *   <li>2. unwrap: ServerHello/Cert/ServerHelloDone
   *   <li>3. wrap: ClientKeyExchange
   *   <li>4. wrap: ChangeCipherSpec
   *   <li>5. wrap: Finished
   *   <li>6. unwrap: ChangeCipherSpec
   *   <li>7. unwrap: Finished
   * </ul>
   *
   * <p>Handshake is also used during the end of the session, in order to properly close the
   * connection between the two peers. A proper connection close will typically include the one peer
   * sending a CLOSE message to another, and then wait for the other's CLOSE message to close the
   * transport link. The other peer from his perspective would read a CLOSE message from his peer
  ...
  //  log.debug("About to do handshake...");
    ...
    // will send and expects to receive from the other peer respectively. Since the messages to be
    // exchanged will usually be less
    // than 16KB long the capacity of these fields should also be smaller. Here we initialize these
    // two local buffers
          ...
          switch (result.getStatus()) {
            ...
            case CLOSED:
              try {
                ...
                // At this point the handshake status will probably be NEED_UNWRAP so we make sure
                // that peerNetData is clear to read.
                peerNetData.clear();
              } catch (Exception e) {
   ...
   *
   * <p>It first calls {@link SSLEngine#closeOutbound()} which prepares this peer to send its own
   * close message and sets {@link SSLEngine} to the <code>NEED_WRAP</code> state. Then, it
   * delegates the exchange of close messages to the handshake method and finally, it closes socket
   * channel.
   *
