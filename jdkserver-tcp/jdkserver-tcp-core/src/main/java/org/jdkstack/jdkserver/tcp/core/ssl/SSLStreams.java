...
/**
 * given a non-blocking SocketChannel, it produces (blocking) streams which encrypt/decrypt the SSL
 * content and handle the SSL handshaking automatically.
 */
public class SSLStreams {
  ...
  public SSLStreams(SSLContext sslctx, SocketChannel chan, boolean f) throws IOException {
    this.sslctx = sslctx;
    this.chan = chan;
    if (f) {
  ...
  class Parameters {
    ...
    Parameters(InetSocketAddress addr) {
      this.addr = addr;
    }
  ...
  /**
   * This is a thin wrapper over SSLEngine and the SocketChannel, which guarantees the ordering of
   * wraps/unwraps with respect to the underlying channel read/writes. It handles the UNDER/OVERFLOW
   * status codes It does not handle the handshaking status codes, or the CLOSED status code though
   * once the engine is closed, any attempt to read/write to it will get an exception. The overall
   * result is returned. It functions synchronously/blocking
   */
  class EngineWrapper {
  ...
  /**
   * represents an SSL input stream. Multiple https requests can be sent over one stream. closing
   * this stream causes an SSL close input.
   */
  class InputStream extends java.io.InputStream {
    ...
    /**
     * close the SSL connection. All data must have been consumed before this is called. Otherwise
     * an exception will be thrown. [Note. May need to revisit this. not quite the normal close()
     * symantics
     */
  ...
  /**
   * represents an SSL output stream. plain text data written to this stream is encrypted by the
   * stream. Multiple HTTPS responses can be sent on one stream. closing this stream initiates an
   * SSL closure
   */
