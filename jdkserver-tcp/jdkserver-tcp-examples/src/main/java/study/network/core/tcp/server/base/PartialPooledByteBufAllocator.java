package study.network.core.tcp.server.base;
...
public final class PartialPooledByteBufAllocator implements ByteBufAllocator {
  ...
  /**
   * Create a new {@link ChannelHandlerContext} which wraps the given one anf force the usage of
   * direct buffers.
   */
  ...
  private static final class ForceDirectPoooledByteBufAllocator implements ByteBufAllocator {
    ...
    @Override
    public int calculateNewCapacity(int minNewCapacity, int maxCapacity) {
      return PartialPooledByteBufAllocator.INSTANCE.calculateNewCapacity(
