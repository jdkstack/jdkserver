...
/**
 * a class which allows the caller to write up to a defined number of bytes to an underlying stream.
 * The caller *must* write the pre-defined number or else an exception will be thrown and the whole
 * request aborted. normal close() does not close the underlying stream
 */
