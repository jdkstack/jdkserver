 ...
 * returns. Filters are organised in chains, and are associated with HttpContext instances.
 *
 * <p>Each Filter in the chain, invokes the next filter within its own doFilter() implementation.
...
public abstract class Filter {
   ...
   *   <li>filter the request body or the response body, by creating suitable filter streams and
   *       calling {@link HttpExchange#setStreams(InputStream,OutputStream)}
   *   <li>set attribute Objects in the exchange, which other filters or the exchange handler can
   *       access.
   *   <li>decide to either
   *       <ol>
   *         <li>invoke the next filter in the chain, by calling {@link
   *             Chain#doFilter(HttpExchange)}
   *         <li>terminate the chain of invocation, by <b>not</b> calling {@link
   *             Chain#doFilter(HttpExchange)}
   *       </ol>
   *   <li>if option 1. above taken, then when doFilter() returns all subsequent filters in the
   *       Chain have been called, and the response headers can be examined or modified.
   *   <li>if option 2. above taken, then this Filter must use the HttpExchange to send back an
   *       appropriate response
   * </ul>
   *
   * @param exchange the {@code HttpExchange} to be filtered.
   * @param chain the Chain which allows the next filter to be invoked.
   * @throws IOException may be thrown by any filter module, and if caught, must be rethrown again.
   * @throws NullPointerException if either exchange or chain are {@code null}
   */
  public abstract void doFilter(Exchange exchange, Chain chain) throws IOException;
  ...
  /**
   * a chain of filters associated with a HttpServer. Each filter in the chain is given one of these
  ...
  public static class Chain {
     ...
     * filter in the chain. The Filter may decide to terminate the chain, by not calling this
     * method. In this case, the filter <b>must</b> send the response to the request, because the
     * application's exchange handler will not be invoked.
     *
     * @param exchange the HttpExchange
     * @throws IOException let exceptions pass up the stack
     * @throws NullPointerException if exchange is {@code null}
     */
    public void doFilter(Exchange exchange) throws IOException {
      if (!iter.hasNext()) {
